...
class RUILoadGen : public LoadGen {
  public:
    ...
    virtual Txn* NewTxn() {
      ...
      // what is the purpose of this payload?
...
class ReadGen: public LoadGen {
  public:
    ReadGen(int readPerc) 
      {
       readPerc_ = readPerc; 
      }
...
//      int txnType = rand() % 100;
      int key = rand() % UINT64_MAX;
      // what is the purpose of this payload?
/*j      byte* payload = new byte[LENGTH_RECORDS];
      for(int i = 0; i < LENGTH_RECORDS; i++) {
        payload[i] = (byte) i;
      }*/
...
void BenchmarkRead(const vector<LoadGen*>& lg) {
  // Number of transaction requests that can be active at any given time.
  int active_txns = 100;
  deque<Txn*> doneTxns;
...
int main(int argc, char** argv) {
  ...
  cout << "Read only, huge key space." << endl;
  lg.push_back(new ReadGen(100));
/*  lg.push_back(new RUILoadGen(100, 0, 0, 100));
  lg.push_back(new RUILoadGen(100, 0, 0, 100));*/

  Benchmark(lg);

/*j  for (unsigned int i = 0; i < lg.size(); i++)
    delete lg[i];
  lg.clear();*/
