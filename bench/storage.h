...
class Storage {
 public:
  ...
  virtual bool Read(TKey key, TValue* result, int txn_unique_id = 0);

  // Inserts the record <key, value>, replacing any previous record with the
  // same key.
  // Note that the third parameter is only used for MVCC, the default vaule is 0.
  virtual void Write(TKey key, TValue value, int txn_unique_id = 0);
  ...
  virtual double Timestamp(TKey key);
  ...
  virtual void Lock(TKey key) {}
  
  virtual void Unlock(TKey key) {}
  
  virtual bool CheckWrite (TKey key, int txn_unique_id) {return true;}
 ...
 private:
   ...
   unordered_map<TKey, TValue> data_;
  
   // Timestamps at which each key was last updated.
   unordered_map<TKey, double> timestamps_;
};
